...
def init_log():
    ...
    def format_record(record):
        ...
        # Convert absolute path to relative path to project root
    ...
    with st.expander(tr("Basic Settings"), expanded=False):
        ...
        with middle_config_panel:
            ...
            if llm_provider == "ollama":
                ...
                with llm_helper:
                    tips = """
                            ...
                            - **API Key**: Fill in any value, e.g., 123
                            - **Base Url**: Usually http://localhost:11434/v1
                                ...
                                - If `MoneyPrinterTurbo` is deployed via `Docker`, it is recommended to use `http://host.docker.internal:11434/v1`
                            - **Model Name**: Use `ollama list` to view, e.g., `qwen:7b`
                            """
            ...
            if llm_provider == "openai":
                ...
                with llm_helper:
                    tips = """
                            ...
                            > VPN with global traffic mode is required
                            - **API Key**: [Apply here](https://platform.openai.com/api-keys)
            ...
            if llm_provider == "moonshot":
                ...
                with llm_helper:
                    tips = """
                            ...
                            - **API Key**: [Apply here](https://platform.moonshot.cn/console/api-keys)
            ...
            if llm_provider == "oneapi":
                ...
                with llm_helper:
                    tips = """
                        ...
                        - **Base Url**: Enter the base URL for OneAPI
            ...
            if llm_provider == "qwen":
                ...
                with llm_helper:
                    tips = """
                            ...
                            - **API Key**: [Apply here](https://dashscope.console.aliyun.com/apiKey)
            ...
            if llm_provider == "g4f":
                ...
                with llm_helper:
                    tips = """
                            ...
                            > [GitHub open source project](https://github.com/xtekky/gpt4free), free to use GPT models, but **stability is poor**
                            - **API Key**: Fill in any value, e.g., 123
                            - **Base Url**: Leave empty
            ...
            if llm_provider == "azure":
                with llm_helper:
                    tips = """
                            ...
                            > [How to deploy a model](https://learn.microsoft.com/en-us/azure/ai-services/openai/how-to/create-resource)
                            - **API Key**: [Create in Azure Portal](https://portal.azure.com/#view/Microsoft_Azure_ProjectOxford/CognitiveServicesHub/~/OpenAI)
                            - **Base Url**: Leave empty
            ...
            if llm_provider == "gemini":
                ...
                with llm_helper:
                    tips = """
                            ...
                            > VPN with global traffic mode is required
                            - **API Key**: [Apply here](https://ai.google.dev/)
                            - **Base Url**: Leave empty
            ...
            if llm_provider == "deepseek":
                ...
                with llm_helper:
                    tips = """
                            ...
                            - **API Key**: [Apply here](https://platform.deepseek.com/api_keys)
            ...
            if llm_provider == "ernie":
                with llm_helper:
                    tips = """
                            ...
                            - **API Key**: [Apply here](https://console.bce.baidu.com/qianfan/ais/console/applicationConsole/application)
                            - **Secret Key**: [Apply here](https://console.bce.baidu.com/qianfan/ais/console/applicationConsole/application)
            ...
            if tips and config.ui["language"] == "zh":
                st.warning(
                    "For users in China, it is recommended to use **DeepSeek** or **Moonshot** as the LLM provider\n- Accessible in China without VPN\n- Free quota upon registration, usually sufficient"
                )
                st.info(tips)
...
lm_provider = config.app.get("llm_provider", "").lower()
panel = st.columns(3)
left_panel = panel[0]
...
with middle_panel:
    ...
    with st.container(border=True):
        ...
        # Ensure there are voices to select
        if friendly_names:
            ...
            # If no voices available, show a warning
            st.warning(
            tr(
        ...
        # Only show play button if voices are available
        ...
        # Show region and API key input for Azure TTS V2 or V2 voice
        ...
        # Show API key input and info for SiliconFlow
